...
set file=.\data\users.%format%
set request="%url%?project=%projectName%&token=%token%"

echo fullFile:%file%
echo request:%request%

@echo curl %request% -k -X POST -O %file% --data-urlencode "q=SELECT * FROM users" --data-urlencode "format=%format%"
curl %request% -k -X POST -o %file% --data-urlencode "q=SELECT * FROM users" --data-urlencode "format=%format%"

goto OK

:ERR
echo export user data occur error!
