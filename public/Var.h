...
public:
	...
	virtual size_t GetMemoryUsage() const
	{
		...
		case VTYPE_STRING:
			...
			{
				size += strlen(m_stringValue) + 1;
			}
			break;
		case VTYPE_WIDESTR:
			if (m_widestrValue != NULL)
			{
				size += (wcslen(m_widestrValue) + 1) * sizeof(wchar_t);
			}
			break;
		case VTYPE_USERDATA:
...
public:
	...
	CVar(int type, const void* pdata, size_t size)
	{
		Assert(type == VTYPE_USERDATA);
	...
	virtual IVarTable* TableVal() const
	{
		switch (m_nType)
		{
		case VTYPE_TABLE:
	...
	virtual IVar& GetChild(const char* key)
	{
		SetTable();

		IVarTable* p = TableVal();
		const IVarPointer* pVarPointer = p->GetVarPointer(key);

		if (NULL == pVarPointer)
		{
			pVarPointer = p->Add(key, IVarPointer(NEW CVar));
		}
	...
	virtual size_t GetMemoryUsage() const
	{
		...
		case VTYPE_USERDATA:
			...
			break;
		case VTYPE_TABLE:
			if (m_tableValue != NULL)
			{
				size += m_tableValue->GetMemoryUsage();
			}
			break;
		default:
			break;
...
private:
	...
	void InnerSetWideStr(const wchar_t* value)
	{
		...
		wchar_t* p = NEW wchar_t[SIZE1];
		memcpy(p, value, SIZE1 * sizeof(wchar_t));
		m_widestrValue = p;
	}

	void InnerSetUserData(const void* pdata, size_t size)
	{
		char* p = NEW char[IVar::GetRawUserDataSize(size)];
		IVar::InitRawUserData(p, pdata, size);
		m_userdataValue = p;
	}

	void InnerSetTable(IVarTable* value)
	{
		CVarTable* p;
		
		if (value != NULL)
		{
			p = NEW CVarTable(*value);
		}
		else
		{
			p = NEW CVarTable;
