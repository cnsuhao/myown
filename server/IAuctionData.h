...
#ifndef _SERVER_IAUCTIONDATA_H
#define _SERVER_IAUCTIONDATA_H

#include "../public/Macros.h"
#include "../public/IVarList.h"
#include "IRecord.h"

#define UNUSED_RECORD_ROW_INDEX			0x4FFF4FFF
#define MAX_AUCTION_REC_COLS			256
...
public:
	...
	/// \param name 属性名
	/// \param type 数据类型
	virtual bool AddAttr(const char* name, int type) = 0;
	/// \brief 删除属性
	/// \param name 属性名
	virtual bool RemoveAttr(const char* name) = 0;
	/// \brief 设置属性是否保存
	/// \param name 属性名
	/// \param value 是否保存
	...
	/// \param not_sync 是否不需要同步到场景服务器（此标志不会保存到数据库）
	virtual bool AddRecord(const char* name, int row_max, int cols, 
		const IVarList& col_types, bool not_sync = false) = 0;
	/// \brief 删除表
	/// \param name 表名
	...
	/// \brief 设置表是否不需要同步到场景服务器
	/// \param name 表名
	/// \param value 是否不同步
	virtual bool SetRecordNotSync(const char* name, bool value) = 0;

	virtual size_t GetRecordFreeRow(const char* name) = 0;
	...
	// 添加表格配置
	virtual bool AddRecordConfig(const char * name, size_t row_max, size_t col_num, const IVarList& col_types) = 0;

	// 删除表格配置
	virtual bool RemoveRecordCofig(const char * name) = 0;
};

inline IAuctionData::~IAuctionData() {}

#endif // _SERVER_IAUCTIONDATA_H
