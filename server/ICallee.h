...
#ifndef _SERVER_ICALLEE_H
#define _SERVER_ICALLEE_H
...
public:
	...
	/// (ps:角色对象断线的情况下进入场景引擎会主动调OnReady回调，逻辑最好能主动判断角色在线状态进行处理)
	/// \param sender 场景对象
	...
	/// \param pSelf  玩家的对象
	/// \param scene 场景对象ID
	/// \param result 报名是否成功(1:成功,其他值为错误码参见OuterErr.h)
	/// \param async_args 异步参数(原样返回RequestApplyBattle接口async_args参数)
	virtual int OnApplyBattle(IKernel* pKernel, IGameObj* pSelf, 
		const PERSISTID& scene, int result, const IVarList& command){return 0;}

	/// \brief 战场匹配确认回调
	/// \param pSelf  玩家的对象
	/// \param scene 场景对象ID
	/// \param result 匹配是否成功(1:成功,其他值为错误码参见OuterErr.h)
	/// \param roomid 匹配的房间编号
	/// \param async_args 平台报名成功的参数
	virtual int OnMatchBattle(IKernel* pKernel, IGameObj* pSelf, 
		const PERSISTID& scene, int result, int roomid, const IVarList& async_args){return 0;}

	/// \brief 战场传输角色的逻辑回调
	/// \param pSelf 玩家对象，回调结束后仅保存该玩家数据。
	/// \param pMergePlayer 待合并的玩家对象
	/// \param msg	玩家数据
	virtual int OnTransBattleRole(IKernel* pKernel, IGameObj* pSelf,
		const IVarList& command, const PERSISTID& player,
		const char* addrinfo, int result){return 0;}
	...
	/// \param room_id  房间id
	/// \param command  结果集
	virtual int OnRequestLeaveRoom(IKernel* pKernel, IGameObj* pSelf, const wchar_t* role_name,
		int room_id, const IVarList& command){return 0;}

	/// \brief 战场发送的custom 消息
	/// \param pSelf 玩家对象
	/// \param command 消息体
	virtual int OnCustomRoom(IKernel* pKernel, IGameObj* pSelf,const IVarList& command){return 0;}

	/// \brief 平台发送的custom 消息
	/// \param pSelf 玩家对象
	/// \param command 消息体
	...
	/// \param num 赠品数量
	/// \param describe 赠品描述
	virtual int OnQueryGiftEx(IKernel* pKernel, IGameObj* pSelf, 
		const wchar_t* player, const char* uid, int id, const char* name, 
		int num,const char * describe) { return 0; }
	/// \brief 收到拍卖行数据系统的自定义信息
	/// \param pSelf 场景对象
	/// \param msg 消息参数
	virtual int OnAuctionMessage(IKernel* pKernel, IGameObj* pSelf,
		const IVarList& msg) { return 0; }
};

inline ICallee::~ICallee() {}
