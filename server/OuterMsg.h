...
// 设备id长度
const size_t DEVICED_ID_LEN_LENGTH = 64;
...
// 位置数据
struct outer_posi_grid_t
{
	unsigned int index			:22;

	unsigned int Orient					:10;
...
// 对象的移动目标
struct outer_object_dest_grid_t
{
	outer_object_t ObjectId;	// 对象标识
	outer_dest_grid_t dest;		// 移动目标
};
...
struct client_stub_msg_base_t: public client_msg_base_t
{
	...
	unsigned short nSafe;		// 是否可靠 1 可靠
};
...
enum LOGIN_TYPE_ENUM
{
	...
	LOGIN_TYPE_GM	        = 200,// gm串登录
	LOGIN_TYPE_GM_RECONNECT	= 201,// gm重连串登录
...
struct client_msg_login_t: public client_msg_base_t
{
	...
	//int	login_type		// 登录类型
	//bool in_stub			//当前是否在副本中(必填)
	//string deviced_uid    //设备id
	//string prop_md5		// 属性md5验证
	//string recd_md5		// 表格md5验证
	//string role_name		// 角色名(utf-8编码)
	//string role_info		// 角色坐标及其朝向（scene_id,x,y,z,orient）
	//int	OsType			//客户端操作系统类型
};
...
struct client_msg_beat_t: public client_msg_base_t
{
};
// 消息延时检测请求
struct  client_msg_tracert_t: public client_msg_base_t
{
	unsigned int clietstamp; // 客户端的时间戳，精确到毫秒
	unsigned char type;		  // 测试类型，默认为0(场景延时)，以后可以完善(登录、存档等) 
	unsigned int serial;	  // 客户端测试消息序列串号
	unsigned short nArgNum;	 // 参数数量
};
...
enum SERVER_MESSAGE_ENUM
{
	...
	SERVER_MSG_BATCH_COMPRESS,			// 这个消息的枚举值不能是0xEE ，因为使用地方没对他做转意
	SERVER_CUSTOM_MERGE,
	SERVER_MESSAGE_MAX,
};
...
struct server_msg_batch_compress_t
{
    unsigned char nEndChar1;
	unsigned char nEndChar2;
	unsigned char nMsgType;		// 消息类别
};

// 语音的 key
struct server_msg_voice_key_t: public server_msg_base_t
{
	//const char *key;
};
...
struct server_msg_version_t: public server_msg_base_t
{
	unsigned char nUseSimpleProtocal;		// 精简协议
	unsigned char nBeatInterval;
	unsigned char nCheckInterval;
	unsigned char byMsgEncoder;				// 消息是否加密
	unsigned char nBeatType;                //心跳类型 0:不使用心跳检测,1:断线检测心跳. 1:消息延迟检测类心跳
	//const char *ip;
};

struct server_msg_entry_stub_t: public server_msg_base_t
{
	unsigned short nArgNum;		// 参数数量
	//unsigned char arg_type;	// 参数类型
	// 参数数据
...
struct server_msg_exit_stub_t: public server_msg_base_t
{
	unsigned short nArgNum;		// 参数数量
	//unsigned char arg_type;	// 参数类型
	// 参数数据
...
// 对象位置和目标
struct server_msg_location_grid_t: public server_msg_base_t
{
	outer_object_t ObjectId;	// 对象标识
	outer_posi_grid_t posi;			// 位置
};
...
// 对象运动目标
struct server_msg_moving_grid_t: public server_msg_base_t
{
	outer_object_t ObjectId;	// 对象标识
	outer_dest_grid_t dest;			// 移动目标
};
...
// 多个对象的目标同步
struct server_msg_all_dest_grid_t: public server_msg_base_t
{
	unsigned short nCount;			// 对象数量
	outer_object_dest_grid_t	info[1];	// 对象移动目标
};
// 多个对象的目标同步(带消息延时)
...
struct server_msg_beat_t: public server_msg_base_t
{
};
// 消息延时检测回应
struct server_msg_tracert_t: public server_msg_base_t
{
	unsigned int clietstamp; // 客户端的时间戳，精确到毫秒
	unsigned char type;		  // 测试类型，默认为0(场景延时)，以后可以完善(登录、存档等)
	unsigned int serial;	  // 客户端测试消息序列串号
	...
	//unsigned char hoptype1; // 当前模块类型
	//unsigned int hopstamp1;// 当前模块的时间戳，GetTickCount()
	//....
	//unsigned char hoptypen;
	//unsigned int hopstampn;
};

enum TRACERT_MODULE_ENUM
{
	TRACERT_ENTRY_NETWOK,			// 接入服务器网络模块
	TRACERT_ENTRY_PROCESS,			// 接入服务器处理模块
	TRACERT_WORLD_NETWOK,			// 世界服务器网络模块
	TRACERT_WORLD_PROCESS,	        // 世界服务器场景处理模块
	TRACERT_SERVER_NETWOK,			// 游戏服务器网络模块
	TRACERT_SERVER_SCENEPROCESS,	// 游戏服务器场景处理模块
	TRACERT_ENTRY_RETURNNETWORK,	// 接入服务器返回消息的网络模块

	TRACERT_ROOM_ENTRYNETWOK,		// 房间服务器接入网络模块
	TRACERT_ROOM_ENTRYPROCESS,		// 房间服务器接入处理模块
	TRACERT_ROOM_QUEUEPROCESS,		// 房间服务器消息队列模块
	TRACERT_ROOM_SCENEPROCESS,		// 房间服务器场景模块
	TRACERT_ROOM_ENTRY_RETURNNETWORK,// 房间服务器场景模块
};

//数据验证
struct client_stub_msg_check_data
{
	int			nargs;				// 参数格式
	char		szMsgId[64+1];		// 消息id
};

struct client_stub_msg_check_data_ack
{
	int		nRes;				//	结果
	char	szMsgId[64+1];		//	消息标识
};
...
{
	size_t dTime;				//发送时间
	size_t nLastSendTime;		//上次发送的时间
	short  nMsgType;			//
	void* pMsg;					//消息内容 varlist
};
//本地副本的消息 ->客户端
enum STUB_MESSAGE_ENUM
{
	STUB_MSG_TO_CLIENT = SERVER_MSG_STUB_TO_CLIENT + 100,// 本地服消息到client
	STUB_MSG_EXITED_STUB,			//本地副本通知客户端退出本地副本
	STUB_MSG_MAX,
};
//客户端，远程服务器 <—>本地副本的子消息
enum STUB_SUB_MESSAGE_ENUM
{
	SERVER_MSG_STUB_SAFE_ACK,	// 数据验证回复
...
struct stub_msg_exited_stub: public server_msg_base_t
{
